...
#define POKEMON_DELAY 4000
#define RAINBOW_OVERFLOW 7
void pokemonMode(){
  counter++;
  if(counter>COUNTER_LIMIT) counter=COUNTER_RESET, page=increaseValue(page,RAINBOW_OVERFLOW);
  hw.setColor(page);
  ...
  for(int i=ZERO;i<NUMBER_OF_BIG_BUTTONS;i++) if(hw.buttonState(SMALL_BUTTON_2) && hw.justPressed(i)) loadPreset(i);
